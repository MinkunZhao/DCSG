...
class AdaptiveMask(nn.Module):
    ...
    def forward(self, head_embeds, tail_embeds):
        ...
        A_tensor = torch_sparse.SparseTensor(row=self.head_list, col=self.tail_list, value=edge_alpha,
                                             sparse_sizes=self.matrix_shape).cuda()
...
class KMeansClustering(nn.Module):
    ...
    def forward(self, embeds):
        ...
        for i in range(1000):
            dists = (embeds.view([-1, 1, self.embedding_size]) - centroids.view(
                [1, -1, self.embedding_size])).square().sum(-1)
