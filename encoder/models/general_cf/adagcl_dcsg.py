...
class AdaGCL_dcsg(BaseModel):
    ...
    def forward(self, adj=None, keep_rate=1.0, return_channels=False):
        if not self.is_training and self.final_embeds is not None and not return_channels:
    ...
    def cal_loss(self, batch_data):
        ...
        user_cf, item_cf, user_sem, item_sem, user_embeds, item_embeds, gate = self.forward(
            self.adj, self.keep_rate, return_channels=True
        )
