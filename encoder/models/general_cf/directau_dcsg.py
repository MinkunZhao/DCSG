...
from encoder.models.loss_utils import alignment, uniformity, cal_infonce_loss
...
class DirectAU_dcsg(BaseModel):
    def __init__(self, data_handler):
        super(DirectAU_dcsg, self).__init__(data_handler)
        # 原始协同图参数
        ...
        self.gamma = self.hyper_config['gamma']  # DirectAU特有参数
    ...
    def _propagate_co(self, adj, embeds):
        ...
        # DirectAU使用平均而不是求和
        return sum(embeds_list) / len(embeds_list)
    ...
    def forward(self, keep_rate=1.0):
        ...
        # 应用edge dropping
        dropped_adj = self.edge_dropper(self.adj, keep_rate) if self.is_training else self.adj
    ...
    def cal_loss(self, batch_data):
        ...
        gate_loss /= 2.0  # 平均两个部分的损失
        gate_loss *= self.gate_supervision_weight  # 应用权重系数

        # 总损失
        loss = directau_loss + kd_loss + contrast_loss + gate_loss
        losses = {
