...
class SimGCL_dcsg(BaseModel):
    def __init__(self, data_handler):
        super(SimGCL_dcsg, self).__init__(data_handler)
        # 原始协同图参数
    ...
    def forward(self, perturb=False, keep_rate=1.0, return_channels=False):
        # 协同通道
    ...
    def cal_loss(self, batch_data):
        ...
        user_embeds1, item_embeds1, _ = self.forward(perturb=True, keep_rate=self.keep_rate)
        user_embeds2, item_embeds2, _ = self.forward(perturb=True, keep_rate=self.keep_rate)
        user_cf, item_cf, user_sem, item_sem, user_embeds3, item_embeds3, gate = self.forward(
            perturb=False, keep_rate=self.keep_rate, return_channels=True
        )

        anc1, pos1, _ = self._pick_embeds(user_embeds1, item_embeds1, batch_data)
        anc2, pos2, _ = self._pick_embeds(user_embeds2, item_embeds2, batch_data)
        ...
        fused_embeds = t.cat([user_embeds3, item_embeds3], dim=0)
        node_ids = t.cat([anc, pos + self.user_num, neg + self.user_num])

        bpr_loss = cal_bpr_loss(anc3, pos3, neg3)
        ...
        cl_loss = self.cl_weight * cl_loss / anc1.shape[0]
        ...
        gate_loss = (
            F.mse_loss(gate[anc], gate_labels[anc])
            + F.mse_loss(gate[self.user_num + pos], gate_labels[self.user_num + pos])
            + F.mse_loss(gate[self.user_num + neg], gate_labels[self.user_num + neg])
        ) / 3.0
        gate_loss = gate_loss * self.gate_supervision_weight
