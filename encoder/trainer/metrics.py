...
class Metric(object):
    ...
    '''def eval(self, model, test_dataloader):
        ...
        for _, tem in enumerate(test_dataloader):
            ...
            # 新增：生成用户-商品样本对
            batch_users = test_user
            batch_items = batch_rate.cpu().tolist()  # 转换为Python列表
            for user, items in zip(batch_users, batch_items):
                candidates.extend([(user, item) for item in items])  # 扩展样本对列表
            # print(candidates)
        ...
        return result, candidates  # 返回评估结果'''
